		...
		try { 
			while (true) { 
				...
				//if Login request is recieved
					...
					rs.next();
					System.out.println("Making account object 1");
					...
					rs.next();
					System.out.println("Making account object 2");
					 name = rs.getString("name");
					 cnic = rs.getString("cnic");
					 age = rs.getInt("age");
					 
					 rs=stmt.executeQuery("SELECT * FROM thrifty.address WHERE city_name=\"Lahore\"" );
					rs.next();
					System.out.println("SELECT * FROM thrifty.address WHERE city=Lahore ");
					 cityName = rs.getString("city_name");
					 area = rs.getString("area");
					 houseNo = rs.getString("houseNo");
					 
					 rs=stmt.executeQuery("SELECT * FROM thrifty.contactinfo WHERE email = \"muneeb@gmail.com\"");
						rs.next();
						System.out.println("SELECT * FROM thrifty.contactinfo WHERE userID = \"" + accID + "\"");
					...
					//AccountData acc = new AccountData("khajestaone",khajesta@gmail.com, password,  name, cnic, Integer.toString(age), mobile,  Email,  cityName, area,  houseNo);
					acc.ads = ads;
